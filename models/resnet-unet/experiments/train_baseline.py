...
train_baseline.py
=================
Trains the baseline Res18-UNet on WildfireSpreadTS_10pct for
3 folds x 20 epochs.  Runs T=5 (multitemporal) only.

Dataset layout (pre-split HDF5):
  WildfireSpreadTS_10pct/train/2018+2019  val/2020  test/2021

3-fold CV (test always = 2021):
  Fold 0 : train=[2018,2019]  val=[2020]
  Fold 1 : train=[2018,2020]  val=[2019]
  Fold 2 : train=[2019,2020]  val=[2018]

Results -> experiments/results/baseline_results.json
...
class BaselineSMPModel(SMPModel):
    """Res18-UNet with AdamW optimiser."""
...
def train_one_fold(fold_id, fold, data_root, epochs, batch_size, num_workers, device, results_dir):
    ...
    print(f"  Baseline: no improvements")
    print(f"{'='*60}")
    ...
    print(f"  Model: Res18-UNet | n_channels={N_CHANNELS} | pos_weight={pos_weight:.4f}")

    run_name = f"baseline_T{N_OBS}_fold{fold_id}"
...
def main():
    ...
    print(f"\n{'#'*60}")
    print(f"#  BASELINE  T={N_OBS}  ({len(FOLDS)} folds x {args.epochs} epochs)")
    print(f"{'#'*60}")

    all_results = []
    for fold_id, fold in enumerate(FOLDS):
        r = train_one_fold(fold_id, fold, data_root, args.epochs,
                           args.batch_size, args.num_workers, device, results_dir)
        all_results.append(r)

    aps = [r["test_AP"] for r in all_results]
    print(f"\n  -- BASELINE T={N_OBS} Summary --")
    ...
    print("="*55)
    print(f"  {'Config':<20} {'Fold 0':>8} {'Fold 1':>8} {'Fold 2':>8} {'Mean':>8}")
    print("-"*55)
    row = f"  Baseline T={N_OBS}        " + "".join(f"{a:>8.4f}" for a in aps)
    row += f"  {sum(aps)/len(aps):>6.4f}"
    print(row)
    print("="*55)
